...
:func:`mael_daemon.agent_model.apply_event`, and writes answers back on the
...
agent has an :class:`~mael_daemon.agent_model.AgentSpec` on disk saying what it
...
from mael_agent.agent_transport import STREAM_LIMIT, DaemonPaths, daemon_paths
...
def _take_lock(path: Path) -> int | None:
    ...
    root. Deliberately not :func:`mael_common.util.locked_file`: that is a
...
class AgentDaemon:
    ...
    async def start_agent(
        ...
        *,
        ...
        :func:`~mael_daemon.agent_model.build_agent_env`. A task launch passes
    ...
    async def _attach(
        ...
        *,
        ...
        Opens with an :data:`~mael_agent.agent_wire.AGENT_DETAIL` frame holding
        :func:`~mael_daemon.agent_model.build_agent_detail`. The host knows what
        ...
        past the cursor, a :data:`~mael_agent.agent_wire.TRUNCATED` marker says
        how many events are gone. A :data:`~mael_agent.agent_wire.BACKLOG_END`
        ...
        The stream ends with an :data:`~mael_agent.agent_wire.AGENT_EXITED`
