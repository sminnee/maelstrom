...
and saved as ``agent-daemon/fixtures/agent_events/``.
``docs/dev/agent-daemon.md`` documents the protocol; read it before changing a
shape.
"""
...
from mael_common.util import sanitise_child_env
...
def build_daemon_identity(
    *,
    ...
    module's own path — ``agent-daemon/src/mael_daemon/agent_model.py`` sits
    three directories below the checkout root.
    """
    return DaemonIdentity(
        pid=pid,
        version=version,
        executable=executable,
        source_tree=str(Path(module_file).parents[3]),
        root=root,
