...
def run(**over):
    return close_worktree_fully(
        "myproject", "alpha", WORKTREE_PATH, PROJECT_PATH, steps=steps(**over)
    )


class TestTheSequence:
    def test_a_clean_close_succeeds_and_reports_the_close_message(self):
        result = run()
        assert result.close.success
        assert result.close.message == "Closed"

    def test_a_running_environment_is_stopped_first(self):
        order: list[str] = []
        result = run(
    ...
    def test_no_environment_means_no_stop(self):
        stop = MagicMock(return_value=[])
        run(env_status=lambda p, w: None, stop_env=stop)
        stop.assert_not_called()

    def test_a_dead_environment_means_no_stop(self):
        stop = MagicMock(return_value=[])
        run(env_status=lambda p, w: [MagicMock(alive=False)], stop_env=stop)
        stop.assert_not_called()

    def test_daemon_agents_stop_before_the_session_pids_are_signalled(self):
        order: list[str] = []
        result = run(
            stop_agents=lambda path: order.append("daemon") or ["agent a1: stopped"],
    ...
    def test_env_vars_are_copied_back_before_the_close(self):
        order: list[str] = []
        run(
    ...
    def test_the_cmux_workspace_is_closed_after_a_successful_close(self):
        result = run(close_workspace=lambda p, w: True)
        assert any(
...
class TestTheCopyBackSplit:
    def test_the_rescue_is_reported_before_the_close_is_announced(self):
...
class TestARefusal:
    def test_a_refused_close_leaves_the_cmux_workspace_open(self):
    ...
    def test_the_refusal_message_is_the_model_s_own(self):
        result = run(
            close=lambda path, force: CloseResult(
...
class TestForce:
    def test_force_is_threaded_into_the_close(self):
        seen: list[bool] = []
    ...
    def test_without_force_the_close_is_asked_not_to_force(self):
        seen: list[bool] = []
        run(
            close=lambda path, force: (
                seen.append(force) or CloseResult(success=True, message="Closed")
...
class TestWarnings:
    def test_a_copy_back_result_is_reported_but_never_fails_the_close(self):
        result = run(copy_back=lambda pp, p: CopyBackResult(added={"API_KEY": "x"}))
        assert result.close.success
        assert result.copy_back.added == {"API_KEY": "x"}

    def test_no_project_path_skips_the_copy_back(self):
