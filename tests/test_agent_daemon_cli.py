...
from maelstrom import agent_daemon_cli, cli
...
def run_cli(argv: list[str], replies: list[dict] | None = None):
    ...
    return drive(agent_daemon_cli.cmd_daemon, argv, replies)


def test_mael_agent_carries_the_daemon_group():
    """`cli` adds the group; `agent_cli` itself knows nothing of the daemon."""
    result = CliRunner().invoke(cli.cli, ["agent", "daemon", "--help"])
