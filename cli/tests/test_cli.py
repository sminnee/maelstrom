...
class TestNonProductionWarning:
    def test_unmarked_commands_name_the_active_data_roots(self, monkeypatch):
        ...
        with patch("mael_cli.cli.ensure_cmux_running", return_value=True):
...
class TestResolvePr:
    ...
    def test_list_all_reads_the_batch_not_one_call_per_row(self):
        ...
        with patch("mael_cli.cli.load_global_config") as mock_config:
...
class TestRemoveMultiTarget:
    ...
    def test_rm_multiple_worktrees(self):
        ...
        with patch("mael_cli.cli.resolve_context") as mock_resolve:
            with patch("mael_cli.cli.get_worktree_dirty_files", return_value=[]):
                with patch(
    ...
    def test_rm_stops_the_daemon_s_agents_before_any_pid_is_signalled(self):
        ...
        with patch("mael_cli.cli.resolve_context") as mock_resolve:
            ...
            with (
                patch("mael_cli.cli.get_worktree_dirty_files", return_value=[]),
...
class TestCloseWait:
    ...
    def test_wait_merged_proceeds_to_close(self):
        ...
        with (
            ...
            patch("mael_domain.worktree_close.get_env_status", return_value=None),
            patch("mael_cli.cli.wait_for_merge") as mock_wait,
            patch("mael_domain.worktree_close.close_worktree") as mock_close,
        ):
...
class TestStaleSymlinkCleanup:
    ...
    def test_removes_stale_symlink_into_source(self, tmp_path):
        ...
        from mael_cli.claude_integration import _symlink_items
...
class TestCmdAddRecycle:
    ...
    def _setup_recycle_mocks(self, stack, tmp_path, helper_return=([], None)):
        ...
        stack.enter_context(
            patch("mael_cli.cli.detect_add_context", return_value="regular")
        )
        ...
        stack.enter_context(
            patch(
                "mael_cli.cli.extract_worktree_name_from_folder",
                return_value="bravo",
            )
        )
        stack.enter_context(
        ...
        stack.enter_context(patch("mael_cli.cli.launch_add_in_worktree"))
    ...
    def _setup(self, stack, tmp_path, existing=True):
        ...
        mocks = {
            ...
            "launch_add_in_worktree": stack.enter_context(
                patch("mael_cli.cli.launch_add_in_worktree")
            ),
...
class TestCmdAddSync:
    ...
    def _run(self, tmp_path, sync):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch(
                    "mael_cli.cli.setup_worktree_for_branch",
            ...
            stack.enter_context(patch("mael_cli.cli.get_app_url", return_value=None))
            stack.enter_context(
                patch("mael_cli.cli.detect_add_context", return_value="regular")
            )
...
class TestCmdSyncAutorepair:
    ...
    def _run(self, args, sync_result, tmp_path):
        ...
        with ExitStack() as stack:
            stack.enter_context(patch("mael_cli.cli.resolve_context", return_value=ctx))
            plain = stack.enter_context(patch("mael_cli.cli.sync_worktree"))
            repair = stack.enter_context(
                patch(
                    "mael_cli.cli.sync_worktree_with_autorepair",
    ...
    def test_sync_all_without_the_flag_uses_the_plain_sync(self, tmp_path):
        ...
        with ExitStack() as stack:
            ...
            repair = stack.enter_context(
                patch("mael_cli.cli.sync_worktree_with_autorepair")
            )
...
class TestCmuxStatus:
    ...
    def test_ok_when_cmux_reachable(self):
        with (
            patch("mael_cli.cli.ensure_cmux_running", return_value=True),
    ...
    def test_errors_when_cmux_unreachable(self):
        with patch("mael_cli.cli.ensure_cmux_running", return_value=False):
            result = CliRunner().invoke(cli, ["cmux", "status"])
            assert result.exit_code != 0
...
class TestCreateProjectIntegration:
    ...
    def _run(self, tmp_path, repo_name="demo", url_name=None):
        ...
        with (
            patch("mael_cli.cli.load_global_config", return_value=config),
            patch("mael_domain.context.load_global_config", return_value=config),
            patch("mael_cli.cli.create_project_repo", return_value=str(upstream)),
            patch("mael_cli.cli.detect_add_context", return_value="regular"),
...
class TestMvProjectIntegration:
    ...
    def _run(self, tmp_path, projects, args, home=None):
        ...
        with (
            ...
            patch("mael_cli.mv_project_cli.get_maelstrom_dir", return_value=mael_dir),
    ...
    def test_doctor_afterwards_prunes_nothing(self, tmp_path):
        ...
        from mael_cli.doctor import run_doctor
