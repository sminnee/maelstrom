"""Tests for mael_cli.worktree_launcher module."""
...
from mael_cli.worktree_launcher import (
    AddContext,
...
class TestOpenWorktree:
    ...
    def test_open_worktree_success(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            with patch("mael_cli.worktree_launcher.run_cmd") as mock_run:
...
class TestAddLauncher:
    def test_daemon_context_takes_precedence_over_cmux(self):
        with (
            ...
            patch("mael_cli.worktree_launcher.current_client") as cmux,
        ):
    ...
    def test_install_starts_asynchronously_in_the_worktree(self, tmp_path):
        with (
            ...
            patch("mael_cli.worktree_launcher.subprocess.Popen") as popen,
        ):
    ...
    async def test_regular_cli_runs_in_the_prepared_worktree(self, tmp_path):
        with patch("mael_cli.worktree_launcher.subprocess.run") as run:
    ...
    async def test_daemon_receives_the_claude_model_alias(self, tmp_path):
        ...
        with (
            patch("mael_cli.worktree_launcher.daemon_client", return_value=client),
            patch("mael_cli.worktree_launcher.agent_prompt_file", return_value=prompt),
        ):
            assert (
    ...
    async def test_the_daemon_receives_the_execute_model_unresolved(self, tmp_path):
        ...
        client = RecordingDaemonClient(replies=[{"id": "agent-1"}])
        with patch("mael_cli.worktree_launcher.daemon_client", return_value=client):
            assert (
    ...
    async def test_regular_daemon_starts_an_agent_without_cmux(self, tmp_path):
        with (
            ...
            patch("mael_cli.worktree_launcher.subprocess.run") as run,
        ):
            assert await launch_add_in_worktree(
                tmp_path,
                "proj",
    ...
    async def test_driven_cli_leaves_the_worktree_when_cmux_is_unavailable(
        ...
        with (
            patch("mael_cli.worktree_launcher.ensure_cmux_running", return_value=False),
            patch("mael_cli.worktree_launcher.start_agent_in_worktree") as start,
        ):
            assert await launch_add_in_worktree(
                tmp_path,
                "proj",
...
class TestBuildHarnessCommand:
    ...
    def test_codex_plain_command(self):
        assert build_harness_command(harness="codex") == ["codex"]

    def test_codex_ignores_claude_only_options(self):
        assert build_harness_command(
            "auto", "abc-123", model="opus", harness="codex"
        ) == ["codex"]
...
class TestBuildTaskLaunchLineCodex:
    ...
    def test_prompt_is_one_positional_argument(self):
        assert describe(build_task_launch_line("proj", "t1", harness="codex")) == (
            'codex "$(mael task prompt t1 --project proj)"'
        )

    def test_env_prefixes_codex(self):
        line = describe(
            build_task_launch_line(
                "proj", "t1", harness="codex", env={"MAEL_TASK_ID": "t1"}
            )
        )
        assert line.startswith("MAEL_TASK_ID=t1 codex ")
...
class TestLaunchAgentInWorktree:
    ...
    async def test_cmux_is_started_after_the_agent_and_before_the_pane(self):
        ...
        with (
            ...
            patch(
                "mael_cli.worktree_launcher.ensure_cmux_running",
    ...
    async def test_cmux_down_returns_false_and_places_nothing(self):
        ...
        with (
            patch("mael_agent.agent_transport.client_factory", lambda **_: client),
            patch("mael_cli.worktree_launcher.ensure_cmux_running", return_value=False),
            patch("mael_cli.worktree_launcher.open_claude_workspace") as mock_open,
        ):
            assert (
                await launch_agent_in_worktree(Path("/wt/alpha"), "proj", "alpha")
                is False
...
class TestLaunchClaudeInWorktree:
    ...
    async def test_returns_true_when_cmux_up_and_placed(self):
        with TemporaryDirectory() as tmpdir:
            ...
            with (
                ...
                patch("mael_cli.worktree_launcher.run_cmd") as mock_run,
            ):
    ...
    async def test_daemon_harness_routes_to_the_agent_launch(self):
        ...
        with (
            patch("mael_cli.worktree_launcher.ensure_cmux_running", return_value=True),
            patch(
                "mael_cli.worktree_launcher.launch_agent_in_worktree",
                return_value=True,
            ) as mock_agent,
        ):
            placed = await launch_claude_in_worktree(
    ...
    async def test_claude_harness_still_places_the_legacy_pane(self):
        with (
            ...
            patch("mael_cli.worktree_launcher.launch_agent_in_worktree") as mock_agent,
        ):
            placed = await launch_claude_in_worktree(
                Path("/wt/alpha"),
                project="proj",
