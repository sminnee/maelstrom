...
class MvProjectHarness:
    ...
    def run(self, args):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch("mael_cli.mv_project_cli.all_live_sessions", _sweep)
            )
            mock("mael_cli.mv_project_cli.load_env_state", return_value=self.env_state)
            mock(
                "mael_cli.mv_project_cli.load_shared_state",
                return_value=self.shared_state,
            )
            mock("mael_cli.mv_project_cli.make_env_store")
            mock("mael_cli.mv_project_cli.stop_env", return_value=["stopped env"])
            mock(
                "mael_cli.mv_project_cli.stop_sessions",
                return_value=["stopped session"],
            )
            mock(
                "mael_cli.mv_project_cli.stop_shared_services",
                return_value=["stopped shared"],
            )
            mock("mael_cli.mv_project_cli.run_git")
            mock(
...
class TestRewritePathString:
    ...
    def test_rewrites_a_log_path(self, tmp_path):
        ...
        with patch("mael_cli.mv_project_cli.get_maelstrom_dir", return_value=mael_dir):
            result = _rewrite_path_string(
