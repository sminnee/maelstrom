...
| `MAEL_STATE_ROOT` | `~/.maelstrom` | The directory holding the state database, which carries the desk and the task notebook. A worktree's `.env` names `~/.maelstrom/playpens/<worktree>`, so `uv run mael` there reads that worktree's own tasks. Every command run against one says so on stderr. `~` expands, and an empty value reads as unset. Only the database moves — see **Playpen** in `CONTEXT.md`. |
