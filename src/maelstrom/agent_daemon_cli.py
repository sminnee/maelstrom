...
The daemon side of ``mael agent``. ``serve`` runs the daemon, and ``status``,
``reconcile``, ``gc`` and ``list`` read its records against the process table.
It lives apart from :mod:`maelstrom.agent_cli` so that the client imports
...
@click.group("daemon", cls=AsyncGroup)
def cmd_daemon() -> None:
