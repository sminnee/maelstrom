...
def split_row_id(id: str) -> tuple[str, str]:
    ...
    The inverse of :func:`~maelstrom.task_table.row_id`. Split once from the
...
class TaskReader(Protocol):
    ...
    Structural rather than :class:`~maelstrom.task_table.TaskTable`, and
...
class ExportQueue(ABC):
    ...
    async def queue_all(self, entries: list[Queued]) -> None:
        ...
        taking its :class:`~maelstrom.state_db.db.Txn`; this one has no write to
...
def enqueue(txn: Txn, id: str, *, path: str, deleted: bool, queued_at: str) -> None:
    ...
    Sync, and takes a :class:`~maelstrom.state_db.db.Txn` rather than a store,
