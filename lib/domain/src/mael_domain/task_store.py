...
The task notebook's own storage is :mod:`mael_domain.task_table`; this serves the
two things that still live as files under ``tasks`` in the notebook root — see
:mod:`mael_domain.notebook_root` — the wiki, and the markdown task export that
:mod:`mael_domain.task_export` writes.
...
def tasks_root() -> Path:
    ...
    :func:`mael_domain.state_db.paths.get_notebook_path`, by the other route; see
