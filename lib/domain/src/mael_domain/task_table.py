...
:mod:`mael_domain.state_db.migrations.notebook_md`.
"""
...
def task_from_row(row: sqlite3.Row) -> Task:
    """One row as a full-fidelity :class:`~mael_domain.task.Task`.
...
class SqliteTaskTable(TaskTable):
    ...
    async def changed_since(self, since: int) -> TaskChanges:
        ...
        :meth:`~mael_domain.state_db.db.StateDb.removed_since` reads the spine's
    ...
    async def save(self, task: Task) -> None:
        ...
        The enqueue is here rather than in :mod:`mael_domain.task`, because this
...
#: :class:`~mael_domain.state_db.db.Txn` directly.
__all__ = [
    "TABLE",
    "InMemoryTaskTable",
    "SqliteTaskTable",
