...
class TestWaitForMerge:
    def test_returns_when_already_merged(self):
        ...
        with (
            patch("mael_domain.github.get_pr_info", return_value=pr),
            patch("mael_domain.github.get_pr_checks", return_value=[]),
        ):
    ...
    def test_merges_after_polling(self):
        ...
        with (
            patch("mael_domain.github.get_pr_info", side_effect=infos),
            patch(
                "mael_domain.github.get_pr_checks",
                return_value=[_check("ci", "PENDING")],
            ),
            patch("mael_domain.github.time.sleep"),
        ):
            result = wait_for_merge(Path("."), timeout=10, poll_interval=0)
    ...
    def test_closed_unmerged_raises(self):
        with (
            patch("mael_domain.github.get_pr_info", return_value=_pr(state="CLOSED")),
            patch("mael_domain.github.get_pr_checks", return_value=[]),
        ):
    ...
    def test_terminal_failed_check_raises(self):
        with (
            patch("mael_domain.github.get_pr_info", return_value=_pr(state="OPEN")),
            patch(
                "mael_domain.github.get_pr_checks",
...
class TestCreateProjectRepo:
    ...
    def _run_create(self, *args, url="https://github.com/me/proj", **kwargs):
        ...
        seen = []
        with patch(
            "mael_domain.github.run_cmd", side_effect=self._fake_run_cmd(url, seen)
        ):
    ...
    def test_stub_files_exist_when_the_commit_runs(self):
        ...
        with patch("mael_domain.github.run_cmd", side_effect=_run):
    ...
    def test_called_process_error_becomes_a_typed_command_failure(self):
        ...
        with patch("mael_domain.github.run_cmd", side_effect=err):
            with pytest.raises(
    ...
    def test_missing_gh_becomes_a_typed_missing_cli_error(self):
        with patch("mael_domain.github.run_cmd", side_effect=FileNotFoundError()):
...
class TestCreatePrAutorepair:
    ...
    def _run(self, tmp_path, **kwargs):
        ...
        with (
            ...
            patch("mael_domain.github.run_git") as git,
            patch("mael_domain.github.get_current_branch", return_value="feature/work"),
...
class TestGetPrForBranch:
    ...
    @staticmethod
    def _list(payload):
        return patch("mael_domain.github.run_cmd_async", return_value=_ok(payload))
    ...
    async def test_a_failed_call_answers_nothing(self):
        with patch(
            "mael_domain.github.run_cmd_async",
...
class TestGetOpenPrs:
    ...
    async def test_maps_every_branch_it_asked_about(self):
        ...
        with patch("mael_domain.github.run_cmd_async", return_value=_ok(page)):
    ...
    async def test_the_query_asks_for_every_field_the_state_rule_reads(self):
        ...
        with patch(
            "mael_domain.github.run_cmd_async", return_value=_ok(_graphql_page({}))
        ) as run:
    ...
    async def test_one_actions_read_answers_every_branch(self):
        ...
        with patch("mael_domain.github.run_cmd_async", side_effect=answers) as run:
    ...
    async def test_no_branches_asks_nothing_at_all(self):
        ...
        with patch("mael_domain.github.run_cmd_async") as run:
    ...
    async def test_unparseable_output_is_a_failure_not_an_empty_repo(self):
        with patch("mael_domain.github.run_cmd_async", return_value=_ok("not json")):
            assert await get_open_prs(Path("."), {"one"}) is None
    ...
    async def test_a_graphql_error_payload_is_a_failure(self):
        ...
        with patch("mael_domain.github.run_cmd_async", return_value=_ok(errors)):
            assert await get_open_prs(Path("."), {"one"}) is None
...
class TestReadPrRepoInfoFailure:
    ...
    def _patches(repo_info_error):
        return (
            patch("mael_domain.github.get_pr_info", return_value=_pr()),
            patch("mael_domain.github.get_repo_info", side_effect=repo_info_error),
            patch("mael_domain.github.get_pr_checks", return_value=[]),
        )
