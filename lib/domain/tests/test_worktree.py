"""Tests for mael_domain.worktree module."""
...
class TestIsWorktreeClosed:
    ...
    def test_returns_true_when_detached_at_origin_main(self):
        ...
        with patch("mael_domain.worktree.get_worktree_dirty_files", return_value=[]):
            with patch("mael_domain.worktree.get_commits_ahead", return_value=0):
    ...
    def test_returns_false_for_dirty_worktree(self):
        ...
        wt = WorktreeInfo(path=Path("/fake"), branch="feature/test", commit="abc123")
        with patch(
            "mael_domain.worktree.get_worktree_dirty_files", return_value=["file.txt"]
        ):
            assert is_worktree_closed(wt) is False
    ...
    def test_returns_false_for_worktree_with_unpushed_commits(self):
        ...
        wt = WorktreeInfo(path=Path("/fake"), branch="feature/test", commit="abc123")
        with patch("mael_domain.worktree.get_worktree_dirty_files", return_value=[]):
            with patch("mael_domain.worktree.get_commits_ahead", return_value=2):
                assert is_worktree_closed(wt) is False
...
class TestClosedWorktrees:
    ...
    def test_a_branch_is_never_closed_and_costs_no_subprocess(self):
        ...
        with patch("mael_domain.worktree.run_cmd") as run:
    ...
    def test_detached_clean_and_contained_is_closed(self):
        ...
        with (
            patch("mael_domain.worktree.get_worktree_dirty_files", return_value=[]),
            patch("mael_domain.worktree._commits_ahead_batch", return_value={"aaa": 0}),
        ):
    ...
    def test_a_dirty_worktree_is_not_closed(self):
        ...
        with (
            patch(
                "mael_domain.worktree.get_worktree_dirty_files", return_value=["f.txt"]
            ),
            patch("mael_domain.worktree._commits_ahead_batch", return_value={"aaa": 0}),
        ):
            assert closed_worktrees(Path("/p"), wts) == set()
    ...
    def test_a_worktree_ahead_of_main_is_not_closed(self):
        ...
        with (
            patch("mael_domain.worktree.get_worktree_dirty_files", return_value=[]),
            patch("mael_domain.worktree._commits_ahead_batch", return_value={"aaa": 3}),
        ):
            assert closed_worktrees(Path("/p"), wts) == set()
    ...
    def test_a_worktree_ahead_of_main_costs_no_git_status(self):
        ...
        wts = [WorktreeInfo(path=Path("/a"), branch="", commit="aaa")]
        with (
            patch("mael_domain.worktree.get_worktree_dirty_files") as dirty,
            patch("mael_domain.worktree._commits_ahead_batch", return_value={"aaa": 3}),
        ):
            assert closed_worktrees(Path("/p"), wts) == set()
    ...
    def test_an_unclassified_worktree_costs_no_git_status(self):
        ...
        with (
            patch("mael_domain.worktree.get_worktree_dirty_files") as dirty,
            patch("mael_domain.worktree._commits_ahead_batch", return_value={}),
        ):
            assert closed_worktrees(Path("/p"), wts) == set()
        dirty.assert_not_called()
    ...
    def test_it_agrees_with_the_per_worktree_function(self):
        ...
        with (
            patch("mael_domain.worktree.get_worktree_dirty_files", return_value=[]),
            patch("mael_domain.worktree._commits_ahead_batch", return_value=ahead),
        ):
        ...
        with patch("mael_domain.worktree.get_worktree_dirty_files", return_value=[]):
            for wt in wts:
                with patch(
                    "mael_domain.worktree.get_commits_ahead",
...
class TestCommitsAheadBatch:
    ...
    def test_a_commit_reachable_from_origin_main_is_not_ahead(self):
        ...
        with patch("mael_domain.worktree.run_cmd", return_value=out):
    ...
    def test_it_classifies_a_mixed_set_in_one_call(self):
        ...
        with patch("mael_domain.worktree.run_cmd", return_value=out) as run:
    ...
    def test_a_missing_origin_main_reads_as_not_ahead(self):
        ...
        with (
            patch("mael_domain.worktree.run_cmd", return_value=out),
            patch("mael_domain.worktree._has_origin_main", return_value=False),
        ):
            assert _commits_ahead_batch(Path("/p"), ["aaa"]) == {"aaa": 0}
    ...
    def test_a_failed_call_with_origin_main_present_classifies_nothing(self):
        ...
        with (
            patch("mael_domain.worktree.run_cmd", return_value=out),
            patch("mael_domain.worktree._has_origin_main", return_value=True),
        ):
...
class TestFindClosedWorktree:
    ...
    def test_returns_none_when_no_worktrees(self):
        ...
        with patch("mael_domain.worktree.list_worktrees", return_value=[]):
    ...
    def test_returns_none_when_no_closed_worktrees(self):
        ...
        with patch("mael_domain.worktree.list_worktrees", return_value=[wt]):
            with patch("mael_domain.worktree.is_worktree_closed", return_value=False):
                result = find_closed_worktree(Path("/fake/project"))
                assert result is None
    ...
    def test_returns_closed_worktree(self):
        ...
        with patch("mael_domain.worktree.list_worktrees", return_value=[wt]):
            with patch("mael_domain.worktree.is_worktree_closed", return_value=True):
                result = find_closed_worktree(Path("/fake/project"))
                assert result == wt
    ...
    def test_skips_project_root(self):
        ...
        with patch(
            "mael_domain.worktree.list_worktrees", return_value=[wt_root, wt_alpha]
        ):
            with patch("mael_domain.worktree.is_worktree_closed", return_value=True):
...
class TestFindWorktreeByBranch:
    ...
    def test_skips_worktree_outside_the_project(self):
        ...
        with patch(
            "mael_domain.worktree.list_worktrees", return_value=[stray, managed]
        ):
    ...
    def test_returns_none_when_only_unmanaged_matches(self):
        ...
        with patch("mael_domain.worktree.list_worktrees", return_value=[stray]):
    ...
    def test_skips_folder_that_is_not_nato_named(self):
        ...
        with patch("mael_domain.worktree.list_worktrees", return_value=[odd]):
            assert find_worktree_by_branch(self.project, "fix/db-hardening") is None
    ...
    def test_excludes_main_worktree_by_default(self):
        ...
        with patch("mael_domain.worktree.list_worktrees", return_value=[main_wt]):
...
class TestSetupWorktreeForBranch:
    ...
    def test_run_install_false_skips_install_on_create(self, git_repo_with_remote):
        ...
        with patch("mael_domain.worktree.run_install_cmd") as install:
            result = setup_worktree_for_branch(
    ...
    def test_idempotent_reuse(self, git_repo_with_remote):
        ...
        with (
            patch("mael_domain.worktree.run_install_cmd") as install,
            patch("mael_domain.worktree.update_claude_local_md") as local_md,
        ):
...
class TestUpdateClaudeLocalMd:
    ...
    def test_creates_claude_local_md(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("mael_domain.ports.get_app_url", lambda *a: None)
...
class TestReclaimOrAllocatePorts:
    ...
    def test_allocates_new_ports_when_old_taken(self, tmp_path, monkeypatch):
        ...
        with patch("mael_domain.ports.check_ports_free", return_value=True):
            reclaim_or_allocate_ports(project_path, worktree_path, "alpha")
...
class TestRegenerateEnvFile:
    ...
    @patch("mael_domain.worktree.write_env_file")
    @patch("mael_domain.worktree.record_port_allocation")
    @patch(
        "mael_domain.worktree.generate_port_env_vars",
        return_value={"PORT_BASE": "300", "APP_PORT": "3000"},
    )
    @patch("mael_domain.worktree.allocate_port_base", return_value=300)
    @patch("mael_domain.worktree.get_port_allocation", return_value=None)
    @patch("mael_domain.worktree.load_config_or_default")
...
class TestStaleWorktreeHandling:
    ...
    def test_list_worktrees_filters_stale_entries(self, tmp_path, capsys):
        ...
        with patch("mael_domain.worktree.run_git") as mock_run_git:
...
class TestTheAsyncReaders:
    ...
    def test_it_reads_the_dirty_files(self, project_with_worktree):
    ...
    def test_a_clean_worktree_has_no_dirty_files(self, project_with_worktree):
        _, worktree_path, _ = project_with_worktree

        assert asyncio.run(get_worktree_dirty_files_async(worktree_path)) == []

    def test_it_counts_the_commits_ahead_of_main(self, project_with_worktree):
        _, worktree_path, _ = project_with_worktree
    ...
    def test_a_pushed_commit_is_not_local_only(self, project_with_worktree):
        _, worktree_path, _ = project_with_worktree
        create_commit(worktree_path, "a.txt", "a\n", "Pushed")
    ...
    def test_an_unpushed_branch_has_no_pushed_count(self, project_with_worktree):
        _, worktree_path, _ = project_with_worktree
    ...
    def test_it_counts_the_pushed_commits(self, project_with_worktree):
        _, worktree_path, _ = project_with_worktree
        create_commit(worktree_path, "a.txt", "a\n", "Pushed")
        git(worktree_path, "push", "origin", "feature/work:feature/work")
        git(worktree_path, "fetch", "origin")
    ...
    def test_it_lists_the_worktrees(self, project_with_worktree):
    ...
    def test_a_worktree_on_a_branch_is_never_closed(self, project_with_worktree):
    ...
    def test_run_git_async_reads_the_branch(self, project_with_worktree):
        _, worktree_path, _ = project_with_worktree
