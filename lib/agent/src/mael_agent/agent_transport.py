"""Transport to the agent daemon, mirroring ``cmux/client.py``.
...
- :class:`ScriptedAsyncDaemonClient` — the fuller fake: scripted rows and
  attach streams, for a caller that follows agents as well as commanding them.

One transport serves every caller, because the CLI runs on one loop — see
:mod:`mael_common.cli_async`.
...
def denied_message(socket_path: str) -> str:
    ...
    return (
        ...
        "deny a socket connect. Check with `uv run mael-agent-daemon status` "
        "from a maelstrom checkout, outside the sandbox, and run "
        "`mael admin install` if so."
    )
...
@dataclass
class SocketAsyncDaemonClient(AsyncDaemonClient):
...
@dataclass
class ScriptedAsyncDaemonClient(AsyncDaemonClient):
