...
describe('the held-text keys', () => {
  ...
  it('names the new-work dialog with one versioned constant', () => {
    expect(retainedKey.newWork()).toBe('mael.retained.v2.new-work');
  });

  it('keys a message input per agent, so two agents never share held text', () => {
    expect(retainedKey.message('d9a4c7f1')).toBe('mael.retained.v2.message.d9a4c7f1');
    expect(retainedKey.message('d9a4c7f1.1')).toBe('mael.retained.v2.message.d9a4c7f1.1');
  ...
  it('reads a key of another version as stale, and a current one as live', () => {
    ...
    expect(isStaleRetainedKey('mael.retained.v1.new-work')).toBe(true);
